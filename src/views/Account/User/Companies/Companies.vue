            ...
            <form @submit.prevent="" class="bg-white p-3 px-4 form-container shadow-sm">
                <div class="col-12">
                    <div class="row g-3">
                        ...
                        <div class="col-lg-2">
                            <button @click="getCompanies(1)" :disabled="tempos.isSearching" type="submit"
                ...
                <div class="col-lg-9 min-vh-100">
                    <div v-if="companiesArray.length" class="row g-3 bg-light p-2 pt-0">
                        <div class="col-12">
                            ...
                            <div class="xsmall">
                                Showing page <span class="fw-bold">{{ pagination.currentPage }}/ {{ pagination.totalPages
                                }}</span>
...
const companiesList: any = useStorage('cpm', '')

// const companiesArray = computed(() => {
//     return JSON.parse(companiesList.value).data.company.data
// })
...
async function getCompanies(page = 1) {
    try {
        ...
        companiesList.value = JSON.stringify(resp.data)
...
async function getCompanyDetails(id: string | number) {
    ...
    // commonStore.selectedCompany = companiesArray.value.find(x => x.id == id)
    // commonStore.toggleDetailsModal()

    try {
        tempos.pageIsLoading = true
        const resp = await api.companyDetails(id)
        commonStore.selectedCompany = resp.data.data.company
        commonStore.toggleDetailsModal()
        tempos.pageIsLoading = false
        console.log(resp);
