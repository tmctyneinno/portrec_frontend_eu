                        ...
                        <div class="mb-5">
                            ...
                            <div class="small float-end">
                                ...
                                <router-link class="theme-color text-decoration-none fw-bold"
                                    to="/auth/login/recruiter">Login</router-link>
                            </div>
                        </div>


                        <div class="mb-3 mt-3">
                            <primaryButton btnType="submit" btnMainClass="btn-dark" v-if="!form.isSendingEmail"
...
const currentRecruiter = ref<any>(null)
...
async function sendOTP() {
    ...
    try {
        ...
        const { data } = await api.recruiterForgotPasswordOTP(formData)
        if (!data?.data) {
            useFxn.toast('Email does not exist!', 'error')
            return
        }
        currentRecruiter.value = data.data
...
const handleOnComplete = async (value: string) => {
    ...
    try {
        ...
        formData.append('recruiter_id', currentRecruiter.value.id)
        const { data } = await api.recruiterForgotPasswordVerifyOTP(formData)
        if (data?.error) {
        ...
        router.push({
            ...
            query: {
                cde: btoa(value),
                id: btoa(currentRecruiter.value.id),
