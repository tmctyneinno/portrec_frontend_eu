                ...
                <div class="col-11 col-lg-12 row justify-content-center">
                    <div class="col-md-6 animate__animated animate__fadeIn">
                        ...
                        <form @submit.prevent="submitForm" class="row g-3">
                            ...
                            <div class="col-12">
                                <div class="fw-bold text-muted small col-12">Confirm New Password: </div>
...
const userIdOnRoute = ref<string>('')

onMounted(() => {
    otpOnRoute.value = atob(route.query?.cde as string);
    userIdOnRoute.value = atob(route.query?.id as string);
    if (!otpOnRoute.value || !userIdOnRoute.value) {
        router.push({ path: '/' })
    }
})
...
async function changePassword() {
    try {
        ...
        formData.append('user_id', userIdOnRoute.value)
        formData.append('otp', otpOnRoute.value)

        const { data } = await api.userResetPassword(formData)
